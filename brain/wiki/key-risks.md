...
### P36: Android 11+ AppOps `attributionTag not declared` warning for expo-audio (FIXED 2026-07-09)
- Cause: On Android 11+ (API 30+), every service that uses app-ops (like media playback) must declare an `android:attributionTag` in the manifest. `expo-audio`'s `AudioControlsService` (a media session service) does NOT set this attribute by default. Every time the service is active — which is whenever the BGM is playing or any audio event happens — the Android system logs a warning: `AppOps: attributionTag not declared in manifest of com.vorithstudio.wordguess`. The user noticed these in the log during the login flow (because the BGM was playing in the background while the user opened Settings and tapped Sign In).
- Fix: `scripts/patch-audio-attribution-tag.mjs` adds `android:attributionTag="audioPlayback"` to the `<service>` element in `android/app/src/main/AndroidManifest.xml`. Wired into the npm `postinstall` hook so it survives every `npx expo prebuild` (the manifest is regenerated by prebuild, so the fix is reapplied on each install). The script is idempotent — it short-circuits if the tag is already present.
- Note on other log noise from the login flow (NOT actionable, system-level):
  ...
  - The login itself uses Google Sign-In via `@react-native-google-signin` + `@react-native-firebase/auth`. The auth flow is correct — these are just warnings, not errors. The login should succeed.
- Lesson: Android 11+ added attribution tag requirements that aren't always set by third-party native modules. For Expo modules that ship services (audio, location, etc.), check whether they need a manual manifest attribute, and if so, make the fix idempotent and run it after prebuild.
- Phase: 6 (Post-launch)
