...
|--------|-----------|-------------|-------|
| Home | Home | yes + headerRight nav menu | Mode selection |
| Game | Game | no (full-screen game) | Tile board + keyboard |
| Result | Result | no | Win/loss reveal |
| Stats | Statistics | yes + headerRight nav menu | Personal stats |
| Settings | Settings | yes + headerRight nav menu | Toggles, account |
| Leaderboard | Leaderboard | yes + headerRight nav menu | Ranking lists |
...
- `NavMenuButton` component renders in `headerRight` of all non-game screens
...
type RootStackParamList = {
  Home: undefined;
  Game: { mode: GameMode; wordLength?: number };
  Result: { won: boolean; word: string; attempts: number };
  Stats: undefined;
  Settings: undefined;
  Leaderboard: undefined;
};
```
- Screen props typed via `NativeStackScreenProps<RootStackParamList, 'ScreenName'>`
